...
import MateriaCard from "@/components/MateriaCardAvance";
import { NotebookText} from "lucide-react";
import { getDetalleAvance } from "@/api/avance-academico";
  ...
  useEffect(() => {
    const fetchAvance = async () => {
      try {
        setLoading(true);
        if (user) {
          const data = await getDetalleAvance(user.id.toString());
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-600">Cargando avance academico...</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="p-6">
        <div className="text-center flex flex-row justify-center items-center gap-2">
           <NotebookText className="w-7 h-7 text-slate-600 inline mb-1" />
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {Object.keys(materiasPorSemestre).length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Aún no tienes materias registradas en tu avance academico.</p>
        </div>
      )}
    </div>
  );
...
export default function AvanceAcademicoPage() {
  return (
    <ProtectedRoute>
      <AvanceContent />
    </ProtectedRoute>
